...
std::vector< std::string > aimware_rbot =
{
	_w("kaka"),
	_w("kaka2"),
	_w("kaka3"),
...
void c_menu::init()
{
	...
	c_tab* tab_first = main_window->add_tab( _w( "Rage" ) );
	{
		...
		c_subtab* subtab_two = tab_first->add_subtab(_w("Auto"));
		{
		...
		} tab_first->pushback_subtab(subtab_two);

		c_subtab* subtab_three = tab_first->add_subtab(_w("Scout"));
		{
		...
		c_subtab* subtab_four = tab_first->add_subtab(_w("AWP"));
		{
		...
		c_subtab* subtab_five = tab_first->add_subtab(_w("Heavy pistols"));
		{
		...
		c_subtab* subtab_six = tab_first->add_subtab(_w("Pistols"));
		{
		...
		c_subtab* subtab_seven = tab_first->add_subtab(_w("Other"));
		{
		...
		} tab_first->pushback_subtab(subtab_seven);

	} main_window->pushback_tab( tab_first );

	c_tab* tab_second = main_window->add_tab(_w("Anti-Aim"));
	{
		c_subtab* subtab_one = tab_second->add_subtab(_w("Anti-aim"));
		{
			subtab_one->add_checkbox(_w("Enabled"), _w("aa"));
			subtab_one->add_checkbox(_w("Fake lag when disabed"), _w("aa_fk_dis"));
			subtab_one->add_multi(_w("Additional Fakelag"), &m_choke, &m_choke2);
			subtab_one->add_checkbox(_w("Prevent onshot backtrack"), _w("aa_onsht"));
			subtab_one->add_checkbox(_w("Indicator"), _w("aa_indicator"));
			subtab_one->add_colorpicker(_w(""), _w("aa_indicator_col"), D3DCOLOR_RGBA(100, 50, 150, 255));
			//subtab_one->add_combobox(_w("Aimware desync type"), _w("aa_aimwaredesync_type"), &aimware_type);
			//subtab_one->add_combobox(_w("Aimware desync rbot"), _w("aa_aimwaredesync_rbot"), &aimware_rbot);
			subtab_one->add_keybind(_w("Slowwalk"), _w("aa_slowwalk_key"));
			subtab_one->add_slider(_w("Slowwalk amount"), _w("aa_slowwalk"), 0.f, 100.f, _w("%.2f %%"));

		} tab_second->pushback_subtab(subtab_one);

		c_subtab* subtab_two = tab_second->add_subtab(_w("AA Regular"));
		{
			subtab_two->add_combobox(_w("Pitch"), _w("aa_pitch_type"), &m_pitch_type);
			subtab_two->add_combobox(_w("Yaw/Desync"), _w("aa_desync_enable"), &m_yaw_desync_type);
			subtab_two->add_checkbox(_w("Jitter"), _w("aa_r_jitter"));
			subtab_two->add_checkbox(_w("Random jitter"), _w("aa_r_jitter_random"));
			subtab_two->add_slider(_w("Jitter range"), _w("aa_r_jitter_range"), 0, 360, _w("%d °"));
			subtab_two->add_slider(_w("Jitter swap"), _w("aa_r_jitter_speed"), 0, 64, _w("%d ° per tick"));
			subtab_two->add_checkbox(_w("Freestand"), _w("aa_r_freestand"));
			subtab_two->add_checkbox(_w("Low performance freestanding"), _w("aa_fs_fps"));
			subtab_two->add_checkbox(_w("Antiaim Override"), _w("aa_manual"));
			subtab_two->add_keybind(_w("back"), _w("k_aa_back"));
			subtab_two->add_keybind(_w("left"), _w("k_aa_left"));
			subtab_two->add_keybind(_w("right"), _w("k_aa_right"));
				if (vars::aa.desync_enable.get<int>() == 2)
				{
					subtab_two->add_keybind(_w("desync swap"), _w("aa_desync_swap"));
					subtab_two->add_slider(_w("body lean"), _w("aa_body_lean"), -100, 100, _w("%d °"));
					subtab_two->add_slider(_w("body inverted"), _w("aa_body_inverted"), -100, 100, _w("%d °"));
				}
			subtab_two->add_slider(_w("Fake lag amount"), _w("aa_r_choke"), 0, 14, _w("%d ticks"));
			} tab_second->pushback_subtab(subtab_two);
		c_subtab* subtab_three = tab_second->add_subtab(_w("AA Moving"));
			{
			subtab_three->add_checkbox(_w("Enabled"), _w("aa_m"));
			subtab_three->add_slider(_w("Add"), _w("aa_m_add"), -180, 180, _w("%d °"));

			} tab_second->pushback_subtab(subtab_three);

		c_subtab* subtab_four = tab_second->add_subtab(_w("AA Air"));
			{
			subtab_four->add_checkbox(_w("Enabled"), _w("aa_a"));
			subtab_four->add_slider(_w("Add"), _w("aa_a_add"), -180, 180, _w("%d °"));

			} tab_second->pushback_subtab(subtab_four);

	} main_window->pushback_tab(tab_second);

	c_tab* tab_six = main_window->add_tab(_w("Legit"));
	{
		c_subtab* subtab_one = tab_six->add_subtab(_w("Aimbot"));
		{

		}tab_six->pushback_subtab(subtab_one);
		c_subtab* subtab_two = tab_six->add_subtab(_w("Legit AA"));
		{

		}tab_six->pushback_subtab(subtab_two);
	} main_window->pushback_tab(tab_six);


	c_tab* tab_fourth = main_window->add_tab( _w( "Visuals" ) );
	{
		c_subtab* subtab_one = tab_fourth->add_subtab( _w( "Chams" ) );
		{
		...
		c_subtab* subtab_two = tab_fourth->add_subtab( _w( "Esp" ) );
		{
		...
		c_subtab* subtab_three = tab_fourth->add_subtab( _w( "Glow" ) );
		{
		...
		} tab_fourth->pushback_subtab( subtab_three );

		c_subtab* subtab_four = tab_fourth->add_subtab( _w( "Misc" ) );
		{
		...
		} tab_fourth->pushback_subtab( subtab_four );

	} main_window->pushback_tab(tab_fourth);

	c_tab* tab_five = main_window->add_tab( _w( "Misc" ) );
	{
		c_subtab* subtab_one = tab_five->add_subtab( _w( "Visual" ) );
		{
			...
			subtab_one->add_checkbox(_w("Aspect radio"), _w("m_screen_pocit"));
			//subtab_one->add_checkbox(_w("Watermark"), _w("DrawWatermark")); //crashel ha bekapcs
			subtab_one->add_slider(_w("Screen aspect"), _w("m_screen_size"), 0, 200, _w("%d %%"));
			
		} tab_five->pushback_subtab( subtab_one );
		c_subtab* subtab_two = tab_five->add_subtab( _w( "Movement" ) );
		{
			subtab_two->add_checkbox( _w( "Bhop" ), _w( "m_bhp" ) );
			subtab_two->add_checkbox( _w( "Autostrafer" ), _w( "m_strf" ) );
		} tab_five->pushback_subtab( subtab_two );
	} main_window->pushback_tab(tab_five);

	
	c_tab* tab_seven = main_window->add_tab( _w( "Skins" ) );
	{
		c_subtab* subtab_one = tab_seven->add_subtab( _w( "Inventory" ) );
		{
		...
		} tab_seven->pushback_subtab( subtab_one );
	} main_window->pushback_tab(tab_seven);

	c_tab* tab_eight = main_window->add_tab(_w("LUAs"));
	{
		c_subtab* subtab_one = tab_eight->add_subtab(_w("LUA Scripts"));
		{
			subtab_one->add_listbox(_w("LUA Scripts"), inventorychanger::get().get_list_items(), _w("inventory_list_idx"), 400, 110);
			subtab_one->add_button(_w("Load"), inventory_add);
			subtab_one->add_button(_w("Unload"), inventory_rm, true);
			subtab_one->add_button(_w("Remove"), inventory_save);
		} tab_eight->pushback_subtab(subtab_one);
	} main_window->pushback_tab(tab_eight);

	c_tab* tab_nine = main_window->add_tab(_w("Config"));
	{
		c_subtab* subtab_one = tab_nine->add_subtab(_w("Slots"));
		{
		...
		} tab_nine->pushback_subtab(subtab_one);

	} main_window->pushback_tab(tab_nine);
...
void c_menu::toggle()
{
	...
	if ( prev_state != state )
	{
		...
		_( cl_mouseenable, "cl_mouseenable 0" );
...
void c_menu::draw_cursor()
{
	...
	D3DCOLOR main_color = D3DCOLOR_RGBA( 20, 179, 178, 0 );
	D3DCOLOR back_color = D3DCOLOR_RGBA( 189, 201, 221, 0 );
