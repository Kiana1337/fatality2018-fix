...
void init::get_offsets()
{
	...
	offsets::pred_random_seed = sig( "client.dll", "A3 ? ? ? ? 66 0F 6E 86" ) + 0x1;
	offsets::set_abs_angles = sig( "client.dll", "55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1 E8" );
	offsets::view_render = sig( "client.dll", "8B 0D ? ? ? ? 57 8B 01 FF 50 14 E8 ? ? ? ? 5F" ) + 0x2;
	offsets::view_render_bms = sig( "client.dll", "8D 43 FC B9 ? ? ? ? 50 A1" ) + 0x4;
	offsets::clip_trace_to_players = sig( "client.dll", "53 8B DC 83 EC 08 83 E4 F0 83 C4 04 55 8B 6B 04 89 6C 24 04 8B EC 81 EC ? ? ? ? 8B 43 10" ); // 53 8B DC 83 EC ? 83 E4 ? 83 C4 ? 55 8B 6B 04 89 6C 24 ? 8B EC 81 EC ? ? ? ? 8B 43 10 56 57 52 F3 0F 10 40
	...
	offsets::set_abs_origin = sig( "client.dll", "55 8B EC 83 E4 F8 51 53 56 57 8B F1 E8 ?? ??" ); // 55 8B EC 83 E4 F8 51 53 56 57 8B F1 E8
	...
	offsets::get_sequence_act = sig( "client.dll", "55 8B EC 53 8B 5D 08 56 8B F1 83" );
	...
	g_disable_post_processing = *reinterpret_cast< bool** >( sig( "client.dll", "83 EC 4C 80 3D" ) + 0x5 );
...
void __forceinline __cdecl init::get_interfaces()
{
	...
	g_pGlobals = **reinterpret_cast< IGlobalVarsBase*** > ( ( *reinterpret_cast< uintptr_t** > ( g_pClient ) )[ 0 ] + 0x1F );
	...
	g_pInput = *reinterpret_cast< IInput** >( sig( "client.dll", "B9 ? ? ? ? F3 0F 11 04 24 FF 50 10" ) + 0x1 );
	...
	g_pClientState = **reinterpret_cast<IClientState***> (sig("engine.dll","A1 ? ? ? ? 8B 80 ? ? ? ? C3") + 0x1);
	...
	g_ppGameRules = *reinterpret_cast< C_CSGameRulesProxy*** >( sig( "client.dll", "A1 ? ? ? ? 85 C0 0F 84 ? ? ? ? 80 B8 ? ? ? ? ? 74" ) + 0x1 );
	...
	//g_pHud = *reinterpret_cast< CHud** >( sig( "client.dll", "B9 ? ? ? ? 56 68 ? ? ? ? E8 ? ? ? ?" ) + 0x1 ); // need fix patters
...
void init::hooks()
{
	...
	orig_do_extra_bone_processing = ccsplayer->apply<do_extra_bone_processing_fn>( 198, do_extra_bone_processing );
	orig_eye_angles = ccsplayer->apply<eye_angles_fn>( 165, eye_angles );
	...
	orig_standard_blending_rules = ccsplayer->apply<standard_blending_rules_fn>( 206, standard_blending_rules );
