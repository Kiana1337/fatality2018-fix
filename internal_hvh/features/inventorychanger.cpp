...
#include <internal_hvh\fatality_gui\menu\config\json.h>
#include <fstream>
...
{
	return ( idx == WEAPON_KNIFE || idx == WEAPON_KNIFE_T || idx == WEAPON_BAYONET || idx == WEAPON_KNIFE_BUTTERFLY
...
{
	if ( !inventory )
		inventory = **reinterpret_cast< CPlayerInventory*** >( sig( "client.dll", "8B 0D ? ? ? ? 85 C9 74 18 51" ) + 0x2 );
