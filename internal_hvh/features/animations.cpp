...
bool fresh_tick()
{
	auto _old = g_pGlobals->framecount;
	auto _new = g_pGlobals->framecount;
	if (_old != _new)
		return true;
	else
		return false;
...
void animations::update_custom_anims( C_CSPlayer * player )
{
	...
	static float proper_abs = g_pLocalPlayer->get_anim_state()->m_flGoalFeetYaw;
	...
	/*if (fresh_tick()) 
	{
		if (g_send_packet)
		{
			proper_abs = player->get_anim_state()->m_flGoalFeetYaw;
			backup_poses = player->get_pose_params();
		}
	}*/
	...
	player->set_abs_angles(Vector(0, proper_abs, 0));
	player->get_anim_state()->m_flUnknownFraction = 0.f;
