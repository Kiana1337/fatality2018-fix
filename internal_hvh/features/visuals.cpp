...
//#ifdef _DEBUG
	//render::get().text( Vector2D( 10, 90 ), tfm::format( "%f  :  %f ", g_pEngine->GetNetChannelInfo()->GetLatency( FLOW_OUTGOING ), g_pEngine->GetNetChannelInfo()->GetLatency( FLOW_INCOMING ) ), get_col( vars::aa.indicator_col.get<uintptr_t>() ), fonts::esp_small, c_font::left_aligned | c_font::centered_y | c_font::drop_shadow );
//#endif
}
...
void visuals::player_info( const visual_data_t & player )
{
	...
	const auto info_right = [ & ]()
	{
		...
		if ( player.reload && vars::visuals.esp.reload.get<bool>() )
		{
			_( BOMB, "RELOAD" );
			render::get().text( Vector2D( player.top.x + player.width + 4, player.top.y + 8 * elements++ ), BOMB, Color( 0, 155, 255, player.alpha ), fonts::esp_small, c_font::left_aligned | c_font::drop_shadow );
		}
	};
...
void visuals::queue_world_data()
{
	...
	for ( auto i = 1; i < g_pEntitiyList->GetHighestEntityIndex(); i++ )
	{
		...
		if ( is_weapon( id ) )
		{
			...
			if ( weapon )
				data.name = clean_item_name( weapon->GetClientClass()->m_pNetworkName );//std::string( conv.begin(), conv.end() );
...
void visuals::queue_player_data()
{
	...
	for ( auto& player : players )
	{
		...
		if ( weapon && weapon->get_wpn_data() )
		{
			...
			player.reload = weapon->in_reload();
		}
		...
		{
			//std::wstring conv( weapon->get_diplay_name() );
			player.weapon_name = clean_item_name(weapon->GetClientClass()->m_pNetworkName);//std::string( conv.begin(), conv.end() );
		}
...
void visuals::world_info( const world_data_t & ent ) const
{
	...
	{
		_( time, "%.2fs" );
		render::get().text( Vector2D( ent.pos.x, ent.pos.y + 8 ), tfm::format( time, ent.explode_time ), Color( 255, 0, 0, static_cast< int >( ent.alpha ) ), fonts::esp_small, c_font::centered_x | c_font::centered_y | c_font::drop_shadow );
		defuse_bar( ent );
	}
	...
	if ( ent.classid == ClassId_CPlantedC4 || ent.classid == ClassId_CC4 )
		render::get().text( Vector2D( ent.pos.x, ent.pos.y - 4 ), name, Color( 255, 255, 255, ent.alpha ), fonts::esp_small, c_font::centered_x | c_font::centered_y | c_font::drop_shadow );
	else
		render::get().text( Vector2D( ent.pos.x, ent.pos.y + 4 ), name, color, fonts::esp_small, c_font::centered_x | c_font::centered_y | c_font::drop_shadow );
...
void visuals::draw_scope()
{
	...
	if ( vars::misc.remove_scope.get<bool>() && ( observer && observer->get_scoped() || g_pLocalPlayer && g_pLocalPlayer->get_scoped() ) )
	{
		const auto size = render::get().get_dimensions();
