...
int C_BaseCombatWeapon::get_weapon_type() { if ( !this ) return -1; return this->get_wpn_data()->iWeaponType; }
...
float C_CSPlayer::GetDesyncDelta()
{
	
	auto animstate = this->get_anim_state();

	float duckammount = animstate->m_fDuckAmount;
	float speedfraction = max(0, min((int)animstate->m_flFeetSpeedForwardsOrSideWays, 1));

	float speedfactor = max(0, min(1, (int)animstate->m_flFeetSpeedUnknownForwardOrSideways));
