...
public:
	NETVAR( get_account_id, int, "CBaseAttributableItem->m_iAccountID" )
		...
		NETVAR( get_item_definiton_index, short, "CBaseAttributableItem->m_iItemDefinitionIndex" )
...
public:
	...
	//OFFSET( in_reload, bool, 0x3285)0x32A5
	OFFSET(in_reload, bool, 0x32A5)
...
public:
	...
	OFFSET( get_model_ptr, CStudioHdr*, 0x2950)
	OFFSET( get_enable_invalidate_bone_cache, bool, offsets::enable_invalidate_bone_cache )
	OFFSET( get_writable_bones, int, 0x26B0);	//没更新	骨骼存取器的偏移没变 这个可能也能用
	POFFSET( get_bone_accessor, CBoneAccessor, 0x26A8 )
	POFFSET( get_bone_cache, CBoneCache, 0x2914) /*0x2900*/
...
public:
	...
	//这两个应该坏了 没更新
	OFFSET( get_visual_angles, QAngle, /*0x31D8*/0x31E8) // CCSPlayer->deadflag + 0x4
	//OFFSET( get_context, CIKContext*, 0x266C + 0x4 )

	OFFSET( get_occlusion_flags, int, 0xA28 )
	APOFFSET( get_anim_layers, C_AnimationLayer, 15, 0x29A0)
	OFFSET( get_anim_layer_count, int, 0x299C)
	OFFSET( get_anim_state, CCSGOPlayerAnimState*, 0x9960)
	...
	VFUNC( 190, build_transformations( CStudioHdr* hdr, Vector* vec, Quaternion* q, matrix3x4_t& transform, const int mask, uint8_t* computed ), void( __thiscall* )( void*, CStudioHdr*, Vector*, Quaternion*, matrix3x4_t const&, int, uint8_t* ) )( hdr, vec, q, transform, mask, computed )
	VFUNC( 206, standard_blending_rules( CStudioHdr* hdr, Vector* vec, Quaternion* q, const float time, const int mask ), void( __thiscall* )( void*, CStudioHdr*, Vector*, Quaternion*, float, int ) )( hdr, vec, q, time, mask )
	VFUNC( 224, update_clientside_anim(), void( __thiscall* )( void* ) )( )
